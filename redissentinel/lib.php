...
class cachestore_redissentinel extends cache_store implements cache_is_key_aware, cache_is_lockable,
    ...
    /**
     * Connection to Redis Sentinel for this store.
     *
     * @var Redis
     */
    protected $redis_sentinel;
	
	/**
     * Connection to Redis master server 
	 * to handle writes for this store.
     *
     * @var Redis
     */
	 ...
	 * of the last known master
     *
     * @var Redis
     */
	
	protected $current_master;
	
	/**
     * String containing sentinel master set name
	 * i.e. "mymaster"
     *
     * @var Redis
     */
    ...
    public function __construct($name, array $configuration = array()) {
		...
		if (!array_key_exists('sentinel_server', $configuration) || empty($configuration['sentinel_server'])) {
            return;
        }
		if (!array_key_exists('master_set_name', $configuration) || empty($configuration['master_set_name'])) {
            return;
        }
		...
		$this->master_set = $configuration['master_set_name'];
		$this->password = $password;
		$this->prefix = $prefix;
        $this->redis = $this->new_redis($configuration['server'], $prefix, $password);
        $this->redis_sentinel = $this->new_redis($configuration['sentinel_server'], $prefix, $password);
		$this->redis_master = $this->get_master();
    }
	...
	protected function get_master() {
		$master_array = $this->redis_sentinel->rawCommand('sentinel','get-master-addr-by-name',$this->master_set);
		$master_server = $master_array[0].':'.$master_array[1];
		
		if ($master_server != $this->current_master) {
			return $this->new_redis($master_server,$this->prefix,$this->password);
		}
		else {
    ...
    public function set($key, $value) {
        ...
        return ($master->hSet($this->hash, $key, $value) !== false);
    }
    ...
    public function set_many(array $keyvaluearray) {
        ...
        if ($master->hMSet($this->hash, $pairs)) {
            return count($pairs);
        }
        return 0;
    }
    ...
    public function delete($key) {
		$master = $this->get_master();
        $result = ($master->hDel($this->hash, $key) > 0);
		return $result;
    }
    ...
    public function delete_many(array $keys) {
        ...
        $result = call_user_func_array(array($master, 'hDel'), $keys);
		return $result;
    }
    ...
    public function purge() {
		$master = $this->get_master();
        $result = ($master->del($this->hash) !== false);		
		return $result;
    }
    ...
    public function instance_deleted() {
        $this->purge();
        $this->redis->close();
		$this->get_master()->close();
		$this->redis_sentinel->close();
        unset($this->redis);
    }
    ...
    public function acquire_lock($key, $ownerid) {
		$master = $this->get_master();
        $result = $master->setnx($key, $ownerid);
		return $result;
    }
    ...
    public static function config_get_configuration_array($data) {
        return array(
			...
			'sentinel_server' => $data->sentinel_server,
			'master_set_name' => $data->master_set_name
        );
    }
    ...
    public static function config_set_edit_form_data(moodleform $editform, array $config) {
		...
		$data['sentinel_server'] = $config['sentinel_server'];
		$data['master_set_name'] = $config['master_set_name'];
    ...
    public static function initialise_test_instance(cache_definition $definition) {
        ...
        $configuration['sentinel_server'] = $config->test_sentinel_server;
        $configuration['master_set_name'] = $config->test_master_set_name;
        ...
        $cache = new cachestore_redissentinel('Redis Sentinel test', $configuration);
